...
def make_table_in_data_base():
    cnx = mysql.connector.connect(user='admin', password='123456', host='127.0.0.1', database='car')
    cursor = cnx.cursor()
    query = 'CREATE TABLE all_cars(name varchar(50),year int, milage int, price bigint, id int ); ALTER TABLE all_cars'\
            ' ADD CONSTRAINT uc_all_cars UNIQUE(id);'
...
def search_car_data(link, full_name):
    def insert_data_to_database(full_name, year, milage, price, car_id):
        ...
        cnx = mysql.connector.connect(user='admin', password='123456', host='127.0.0.1', database='car')
        cursor = cnx.cursor()
        query = 'INSERT INTO all_cars value(\'{}\',{},{},{},{})'.format(full_name, int(year), int(milage), int(price),
                                                                        int(car_id[0]))
        try:
